...
"[-1p1s1D?\n@(#)$Id: suimclient (AT&T Labs Research) 2001-09-24 $\n]"
USAGE_LICENSE
"[+NAME?suimclient - client tool for SWIFT message service]"
"[+DESCRIPTION?\bsuimclient\b interfaces with the SWIFT message service to"
" communicate with other SWIFT clients."
"]"
"[999:v?increases the verbosity level. May be specified multiple times.]"
"\n"
"\nhost/port\n"
"\n"
"[+SEE ALSO?\bsuimserver\b(1)]"
;
...
int main (int argc, char **argv) {
    ...
    for (;;) {
        ...
        case -999:
            ...
            continue;
        case '?':
            SUusage (0, "suimclient", opt_info.arg);
            norun = 1;
            continue;
        case ':':
            SUusage (1, "suimclient", opt_info.arg);
            norun = 2;
            continue;
        }
        break;
    ...
    if (argc != 1)
        SUerror ("suimclient", "usage: %s", optusage (NULL));

    if ((cfd = serverget (argv[0])) == -1)
        SUerror ("suimclient", "cannot connect to server");
    FD_ZERO (&infds);
    FD_SET (cfd, &infds);
    FD_SET (0, &infds);
    for (;;) {
        ...
        if ((select (FD_SETSIZE, &outfds, NULL, NULL, NULL)) <= 0)
            SUerror ("suimclient", "select failed");
        if (FD_ISSET (cfd, &outfds)) {
            if ((bufn = read (cfd, bufs, 64 * 1024)) < 0) {
                SUerror ("suimclient", "read from server failed");
                break;
            }
            if (bufn == 0)
                break;
...
static int serverget (char *namep) {
    ...
    if (SUauth (SU_AUTH_CLIENT, cfd, "SWIFTMSERVER", SWIFTAUTHKEY) == -1) {
