...
function vg_cm_customer_check {
    ...
    *)
    ...
    if [[ ${nfs[0]} == '' ]] then
        print "ERROR|customer id is mandatory"
    fi
    if [[ ${nfs[0]} != +([.a-zA-Z0-9_:-]) ]] then
        print "ERROR|the customer id can only contain: . a-z A-Z 0-9 _ : -"
    fi
    if [[
        ${edit.a} == insert &&
        $(egrep "^${nfs[0]}\|" $VGCM_CUSTFILE) != ''
    ]] then
        print "ERROR|customer id ${nfs[0]} already exists"
    fi

    # check customer name field
    if [[ ${nfs[1]} == '' ]] then
        print "ERROR|customer name is mandatory"
    fi
    if [[
        ${edit.a} == insert &&
        $(egrep "\|${nfs[1]}\|" $VGCM_CUSTFILE) != ''
    ]] then
        print "ERROR|customer name ${nfs[1]} already exists"
    fi
