...
function vg_cm_preferences_isallowed {
    typeset ids id
...
function vg_cm_preferences_list {
    typeset ids id i

    ids=$SWMID
    [[ " $SWMGROUPS " == *' vg_att_admin '*  ]] && ids=__ALL__
    for i in $VGCM_PREFERENCESDIR/*.sh; do
        [[ ! -f $i ]] && continue
        id=${i##*/}
        id=${id%.sh}
...
function vg_cm_preferences_check {
    typeset spec=$1
    ...
    *)
    ...
    # check account id
    if [[
        $(egrep "^${nfs[3]};" $VG_DWWWDIR/accounts/current) == ''
    ]] then
        print "ERROR|account id ${nfs[3]} does not exist"
    fi
...
function vg_cm_preferences_pack {
    typeset spec=$1
...
function vg_cm_preferences_split {
    typeset fmt=$1 rec=$2
