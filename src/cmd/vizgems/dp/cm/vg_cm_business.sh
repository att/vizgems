...
function vg_cm_business_check {
    ...
    *)
    ...
    # check business id field
    if [[ ${nfs[0]} == '' ]] then
        print "ERROR|business id is mandatory"
    fi
    if [[ ${nfs[0]} != +([.a-zA-Z0-9_:-]) ]] then
        print "ERROR|the business id can only contain: . a-z A-Z 0-9 _ : -"
    fi
    if [[
        ${edit.a} == insert &&
        $(egrep "^${nfs[0]}\|" $VGCM_BIZFILE) != ''
    ]] then
        print "ERROR|business id ${nfs[0]} already exists"
    fi

    # check business name field
    if [[ ${nfs[1]} == '' ]] then
        print "ERROR|business name is mandatory"
    fi
    if [[
        ${edit.a} == insert &&
        $(egrep "\|${nfs[1]}\|" $VGCM_BIZFILE) != ''
    ]] then
        print "ERROR|business name ${nfs[1]} already exists"
    fi
