function vg_generic_monthly_proc_run {
    typeset -n gv=$1
    typeset datadir=$2
    typeset linkdir=$3
    shift 3
    ...
    for date in $dates; do
        ...
        (
            exec 3< ${gv.lockfile}
            (
                print begin monthly generic:${spec.name} processing $date $(date)
                cd $outdir/total
                $SHELL vg_generic monthly
                print end monthly generic:${spec.name} processing $date $(date)
            ) 2>&1
        )
    done
