function vg_alarm_yearly_proc_run {
    typeset -n gv=$1
    typeset datadir=$2
    typeset linkdir=$3
    shift 3
    ...
    for date in $dates; do
        ...
        (
            exec 3< ${gv.lockfile}
            (
                print begin yearly alarm processing $date $(date)
                cd $outdir/total
                $SHELL vg_alarm yearly
                print end yearly alarm processing $date $(date)
            ) 2>&1
        )
    done
