function vg_stat_monthly_proc_run {
    typeset -n gv=$1
    typeset datadir=$2
    typeset linkdir=$3
    shift 3
    ...
    for date in $dates; do
        ...
        (
            ...
            if [[ ! -f $outdir/avg/total/avg.done ]] then
                (
                    print begin monthly stat prof processing $date $(date)
                    cd $outdir/avg/total
                    $SHELL vg_stat_prof monthly
                    print end monthly stat prof processing $date $(date)
                ) 2>&1
            fi
            (
                print begin monthly stat processing $date $(date)
                cd $outdir/total
                $SHELL vg_stat monthly
                print end monthly stat processing $date $(date)
            ) 2>&1
        )
    done
