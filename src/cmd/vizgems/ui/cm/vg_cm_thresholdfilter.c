...
"[-1p1?\n@(#)$Id: vg_cm_thresholdfilter (AT&T Labs Research) 2009-06-11 $\n]"
USAGE_LICENSE
"[+NAME?vg_cm_thresholdfilter - filter tool for threshold records]"
"[+DESCRIPTION?\bvg_cm_thresholdfilter\b a threshold record at a time, attempts"
...
int main (int argc, char **argv) {
    ...
    for (;;) {
        ...
        case -999:
            ...
            continue;
        case '?':
            SUusage (0, "vg_cm_thresholdfilter", opt_info.arg);
            norun = 1;
            continue;
        case ':':
            SUusage (1, "vg_cm_thresholdfilter", opt_info.arg);
            norun = 2;
            continue;
        }
        break;
    ...
    if (statmapload (getenv ("STATMAPFILE"), FALSE, FALSE, TRUE) == -1) {
        SUwarning (0, "vg_cm_thresholdfilter", "cannot load statmap file");
        noprocessing = TRUE;
    }
    ...
    while ((line = sfgetr (sfstdin, '\n', SF_STRING))) {
        ...
        if (
            ...
            SUwarning (0, "vg_cm_thresholdfilter", "bad record: %s", line);
            sfputr (sfstdout, line, '\n');
            continue;
        }
        *s1++ = 0, *s2++ = 0, *s3++ = 0, *s4++ = 0;
