...
SNMPOID=.1.3.6.1.4.1.13742.6.3.2.2.1.13
$SNMPCMD $SNMPARGS $SNMPOID | tail -1 | read -r line
line=${line##*'STRING: '}; line=${line//\"/}
[[ $line != '' ]] && print "node|o|$aid|si_sysname|$line"

SNMPOID=.1.3.6.1.4.1.13742.6.3.2.1.1.3
$SNMPCMD $SNMPARGS $SNMPOID | tail -1 | read -r line
line=${line##*'STRING: '}; line=${line//\"/}
[[ $line != '' ]] && print "node|o|$aid|si_model|$line"

SNMPOID=.1.3.6.1.4.1.13742.6.3.2.3.1.6
$SNMPCMD $SNMPARGS $SNMPOID | head -1 | read -r line
line=${line##*'STRING: '}; line=${line//\"/}
[[ $line != '' ]] && print "node|o|$aid|si_firmware|$line"

tools='vg_snmp_cmd_mainraritanpx vg_snmp_cmd_mainraritanpxsensor'
