...
tools=vg_snmp_cmd_mainaristaiface

$SNMPCMD $SNMPARGS .1.3.6.1.2.1.25.2.3.1.4.1 | read -r a b c d e
blk=$d
$SNMPCMD $SNMPARGS .1.3.6.1.2.1.25.2.3.1.5.1 | read -r a b c d e
siz=$d
if [[ $blk == +([0-9.]) && $siz == +([0-9.]) ]] then
    (( val = blk * siz ))
