...
main(int argc, char *argv[])
{
    ...
    int             exitval = 0;
#ifdef VGPATCH
    unsigned char **commstrings;
    int commstringn, commstringm, commstringl, commstringi, commstringj;
    int firstvar, argi;

    commstringn = commstringm = commstringl = commstringi = 0;
#endif
...
#ifndef NETSNMP_PARSE_ARGS_SUCCESS
#define NETSNMP_PARSE_ARGS_SUCCESS       0
#endif
#ifndef NETSNMP_PARSE_ARGS_SUCCESS_EXIT
#define NETSNMP_PARSE_ARGS_SUCCESS_EXIT  -2
#endif
#ifndef NETSNMP_PARSE_ARGS_ERROR_USAGE
#define NETSNMP_PARSE_ARGS_ERROR_USAGE   -1
#endif
#ifndef NETSNMP_PARSE_ARGS_ERROR
...
#ifdef VGPATCH
    if (session.community && strncmp (session.community, "SWMUL", 5) == 0) {
        char *s1, *s2, line[100];
        FILE *fp;

        s1 = strdup (session.community) + 5;
        if (!(commstrings = malloc (10 * sizeof (char *)))) {
            fprintf (stderr, "memory: cannot malloc commstrings\n");
            exit (1);
        }
        commstringn = 10;
        while (s1) {
            ...
            if (!*s2)
                s2 = NULL;
            if (commstringm >= commstringn) {
                if (!(commstrings = realloc (
                    commstrings, (commstringn + 10) * sizeof (char *)
                ))) {
                    fprintf (stderr, "memory: cannot realloc commstrings\n");
                    exit (1);
                }
                commstringn += 10;
            }
            if (!(commstrings[commstringm++] = strdup (s1))) {
                fprintf (
                    stderr, "memory: cannot malloc commstring %d\n", commstringm
                );
                exit (1);
            }
            s1 = s2;
        }
        if ((fp = fopen ("snmp.lastmulti", "r"))) {
            fgets (line, 100, fp);
            if (!line[0] || (commstringl = atoi (line)) < 0)
                commstringl = 0;
            fclose (fp);
        }
        if (commstringm < 1) {
            fprintf (stderr, "memory: no password in multi password mode\n");
            exit (1);
        }
        commstringi = 0;
...
#ifdef VGPATCH
            if (firstvar && commstringm > 0 && commstringj != commstringl) {
                FILE *fp;

                if ((fp = fopen ("snmp.lastmulti", "w"))) {
                    fprintf (fp, "%d\n", commstringj);
                    fclose (fp);
                }
            }
...
#ifdef VGPATCH
            if (firstvar && commstringi < commstringm) {
                commstringj = (commstringi++ + commstringl) % commstringm;
                session.community = commstrings[commstringj];
