...
void Mmkcurr (void *p) {
    if (!p || M_AREAOF (p) == otarea[0])
        return;
    if (oti >= otn) {
        otarray[0] = Marraygrow (otarray[0], (long) (2 * otn) * OTSIZE);
        otarray[1] = Marraygrow (otarray[1], (long) (2 * otn) * OTSIZE);
        otn *= 2;
    }
    otarray[0][oti++] = p;
...
void Mdogc (int gctype) {
    ...
    if (gctype == M_GCINCR) {
        if (numofallocs < NUMOFALLOCS / 2) {
            gcsteps = (gcsteps > GCINCRSTEPS) ? gcsteps >>= 1 : GCINCRSTEPS;
            numofallocs = 0;
        }
    }
    if (cbfunc)
...
void Mreport (void) {
    ...
    fprintf (stderr, "live objects:      %8d (", num[0]);
    for (i = 1; i < M_MAXTYPES; i++)
        fprintf (stderr, "%8d%s", num[i], (i == M_MAXTYPES - 1) ? "" : ",");
    fprintf (stderr, ")\n       sizes:      %8d (", siz[0]);
    for (i = 1; i < M_MAXTYPES; i++)
        fprintf (stderr, "%8d%s", siz[i], (i == M_MAXTYPES - 1) ? "" : ",");
    fprintf (stderr, ")\n");
    fprintf (stderr, "free lists: %d\n", freen);
    for (i = 0; i < freen; i++) {
        for (n = 0, fp = freearray[i]; fp; fp = fp->next)
            n++;
        if (n > 0)
            fprintf (stderr, "free list: %d - %d\n", i, n);
    }
#ifdef STATS
