...
"[-1p1s1D?\n@(#)$Id: tiger2ps (AT&T Labs Research) 2001-09-24 $\n]"
USAGE_LICENSE
"[+NAME?tiger2ps - generate ascii format geometry]"
"[+DESCRIPTION?\btiger2ps\b generates postscript geometry files from the"
" specified dataset."
...
#define X(px) ((px) - minp.x) / (double) (maxp.x - minp.x)
#define Y(py) ((py) - minp.y) / (double) (maxp.y - minp.y)
...
static int savepoints (char *dir, char *prefix) {
    ...
    fname = strdup (sfprints ("%s/%s_points.ps", dir, prefix));
