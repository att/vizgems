...
int PASCAL WinMain (
    ...
    if (lpCmdLine[0] == 0)
        sprintf (
            cmd, "%s -e \"load('dotty.lefty');dotty.createviewandgraph(null,'file',null,null);txtview('off');\"", path
        );
    else
        sprintf (
            cmd, "%s -e \"load('dotty.lefty');dotty.createviewandgraph('%Ns','file',null,null);txtview('off');\"",
            path, lpCmdLine
        );
