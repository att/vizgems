...
typedef struct printdict_t {
    Dt_t *nodesleft, *edgesleft, *subgleft, *e_insubg, *n_insubg;
} printdict_t;
...
static Dt_t *Copy;
static int copydictf(Dt_t * d, void *a, void *ignored)
{
    dtinsert(Copy, (Agsym_t *) a);
    return 0;
}

static void copydict(Dt_t * from, Dt_t * to)
{
    Copy = to;
...
static printdict_t *new_printdict_t(Agraph_t * g)
{
    ...
    rv->nodesleft = dtopen(&agNodedisc, Dtoset);
    copydict(g->nodes, rv->nodesleft);
    rv->edgesleft = dtopen(&agEdgedisc, Dtoset);
    copydict(g->outedges, rv->edgesleft);
    rv->n_insubg = dtopen(&agNodedisc, Dtoset);
    rv->e_insubg = dtopen(&agOutdisc, Dtoset);
    rv->subgleft = dtopen(&agNodedisc, Dtoset);
