...
int agcmpin(Dt_t * d, Agedge_t * e0, Agedge_t * e1, Dtdisc_t * disc)
{
...
int agcmpout(Dt_t * d, Agedge_t * e0, Agedge_t * e1, Dtdisc_t * disc)
{
    int e0tailid, e0headid, e1tailid, e1headid;
...
static Agdata_t *agnewdata(void)
{
    ...
    rv = NEW(Agdata_t);
    rv->node_dict = dtopen(&agNamedisc, Dtoset);
...
static Agraph_t *agNEWgraph(char *name, Agraph_t * parent, int kind)
{
    ...
    g->tag = TAG_GRAPH;
    g->kind = kind;
    g->nodes = dtopen(&agNodedisc, Dtoset);
    g->inedges = dtopen(&agIndisc, Dtoset);
    g->outedges = dtopen(&agOutdisc, Dtoset);
...
static int reach0(Dt_t * m, Agnode_t * from, Agnode_t * to)
{
    Agedge_t *e;
